...
import { BUYER_NAV_ITEMS } from "./nav-items";

function NavLink({ href, label, icon: Icon, active, onClick }: {
  href: string;
  label: string;
  icon: (typeof BUYER_NAV_ITEMS)[number]["icon"];
  active: boolean;
  onClick?: () => void;
}) {
  return (
...
// Adapted from CreatorShell.tsx — same desktop-sidebar/mobile-drawer/bottom-nav shape, since a
// buyer's app frame has the exact same structural needs (own nav list, own session, own logout).
export function BuyerShell({ children }: { children: ReactNode }) {
  const pathname = usePathname();
  const { user, logout } = useBuyerSession();
  const [drawerOpen, setDrawerOpen] = useState(false);

  const bottomItems = BUYER_NAV_ITEMS.filter((i) => i.bottomNav);
  ...
  return (
    <div className="min-h-screen bg-bg md:flex">
      <aside className="hidden w-64 shrink-0 border-r border-border bg-surface px-4 py-6 md:flex md:flex-col md:justify-between">
        <div>
          <Link href="/buyer/dashboard" className="mb-8 block font-heading text-xl font-bold text-text-primary">
            {BRAND.name}
          </Link>
        ...
        <div className="flex items-center justify-between border-t border-border pt-4">
          <span className="font-body text-sm text-text-primary">{user?.displayName ?? user?.email ?? user?.phone}</span>
          <button
            type="button"
            onClick={logout}
            aria-label="Chiqish"
      ...
      <header className="flex items-center justify-between border-b border-border bg-surface px-pad-mobile py-3 md:hidden">
        <Link href="/buyer/dashboard" className="font-heading text-lg font-bold text-text-primary">
          {BRAND.name}
        </Link>
        <button
          type="button"
