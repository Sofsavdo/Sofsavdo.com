    ...
    <DataTableShell
      ...
      mobileCards={filtered.map((p) => (
        <MobileDataCard
          key={p.id}
          title={p.creatorName}
          meta={<StatusBadge tone={payoutStatusMeta[p.status].tone} label={payoutStatusMeta[p.status].label} />}
          fields={[
            { label: "Usul", value: p.payoutMethodLabel },
            { label: "So'ralgan", value: new Date(p.requestedAt).toLocaleDateString("uz-UZ") },
            { label: "Summa", value: formatMoneyMinor(p.amountMinor), emphasis: true },
          ]}
          actions={
            <>
...
export default function AdminPayoutsPage() {
  return USE_REAL_API ? <RealAdminPayoutsPage /> : <MockAdminPayoutsPage />;
}
