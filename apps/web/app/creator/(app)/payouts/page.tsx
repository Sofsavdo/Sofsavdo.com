...
import { useAddPayoutMethod, useBalance, usePayoutMethods, usePayouts, useRequestPayout } from "@/services/finance";
      ...
      <Card>
        ...
        {showAddMethod ? (
          <form onSubmit={methodForm.handleSubmit(onAddMethod)} className="flex flex-col gap-3 border-t border-border pt-4" noValidate>
            ...
            />
            {addMethod.isError ? <Alert tone="error">Qo&apos;shishda xatolik yuz berdi.</Alert> : null}
            <div className="flex gap-2">
              <Button type="submit" disabled={addMethod.isPending}>
                {addMethod.isPending ? "Saqlanmoqda..." : "Saqlash"}
              </Button>
        ...
        <CardHeader className="flex-col items-start gap-1">
          ...
          {balance ? (
            <p className="font-body text-sm text-text-secondary">
              Mavjud balans: <strong>{formatMoneyMinor(balance.availableMinor)}</strong> · Minimal:{" "}
              {formatMoneyMinor(balance.minimumPayoutMinor)}
            </p>
          ) : null}
        </CardHeader>
          ...
          <ul className="divide-y divide-border">
            {payouts.map((p) => (
              <li key={p.id} className="flex items-center justify-between py-3">
                ...
                <Badge tone={payoutStatusMeta[p.status].tone}>{payoutStatusMeta[p.status].label}</Badge>
              </li>
            ))}
          </ul>
        )}
